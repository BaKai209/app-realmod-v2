...
export const editAsync = (nombre, producto) => {
    ...
    return async (dispatch) => {
        const collectionTraer = collection(db, "inmuebles");
        ...
        // console.log(id);
        const documRef = doc(db, "inmuebles", id)
...
export const deleteAsync = (nombre) => {
    return async (dispatch) => {
        ...
        traerDatosQ.forEach((docum => {
            deleteDoc(doc(db, "inmuebles", docum.id))
        }))
...
export const listAsync = () => {
    return async (dispatch) => {
        const collectionTraer = await getDocs(collection(db, "inmuebles"));
        const productos = [];
...
export const addAsync = (producto) => {
    return (dispatch) => {
        addDoc(collection(db, "inmuebles"), producto)
            .then(resp => {
...
export const addFormikAsync = (product) => {
    ...
    return (dispatch) => {
        addDoc(collection(db, 'inmuebles'), product)
            .then(resp => {
                console.log(resp);
